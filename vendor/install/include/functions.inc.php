...
function apphp_db_install($sql_dump_file)
{
	...
	*/
	...
	// Define a function to apply stripslashes to all values in an array recursively
	function stripslashes_recursive(&$value)
	{
		// Use stripslashes to remove backslashes from the value
		$value = stripslashes($value);
	}

	// Apply stripslashes_recursive to the $sql_dump array
	array_walk_recursive($sql_dump, 'stripslashes_recursive');
    ...
    $sql_dump = str_ireplace('<PASSWORD>', "'" . $admin_password_hash . "'", $sql_dump);
    // end 2026
...
function draw_side_navigation($step = 1, $draw = true)
{
	$steps = array(
		...
		'5' => array('url' => 'complete_installation.php', 'text' => lang_key('completed'))
	);
