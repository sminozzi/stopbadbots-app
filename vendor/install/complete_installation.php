	...
	if (empty($error_mg)) {
		...
		if (EI_DATABASE_CREATE && ($install_type == 'create') && !$db->Create()) {
			...
			if (EI_CHECK_DB_MINIMUM_VERSION && (version_compare($db->GetVersion(), EI_DB_MINIMUM_VERSION, '<'))) {
				...
				$sql_dump_file = $doc_root . '/stopbadbots/install/' . EI_SQL_DUMP_FILE_CREATE;
				$sql_dir = $doc_root . '/stopbadbots/sql_dump/';
				chmod($sql_dir, 0777);
				chmod($sql_dump_file, 0777);
				$sql_dump = file_get_contents($sql_dump_file);
				chmod($sql_dir, 0755);
				chmod($sql_dump_file, 0755);
				if ($sql_dump != '' or $sql_dump !== false) {
					if (false == ($db_error = apphp_db_install($sql_dump_file))) {
						...
						$api_secret_token = bin2hex(random_bytes(32));

						// Add API_SECRET_TOKEN to config
						$config_file .= "\n";
						$config_file .= "define('API_SECRET_TOKEN', '" . $api_secret_token . "');\n";
                        // end 2026


						$config_file_path = $doc_root . '/' . EI_CONFIG_FILE_DIRECTORY . EI_CONFIG_FILE_NAME; //'config.php';
	...
	<div id="main">
		...
		<div id="content">
			...
			<div class="central-part">
				<h2><?php echo lang_key('step_5_of'); ?>
					<?php if (!$completed) { ?>
						...
						<?php } else { ?>
							...
							<tr>
								<td>
									...
									<br /><br />
									<?php if (EI_APPLICATION_START_FILE != '') { ?><a href="<?php echo '../index.php'; ?> ><?php echo lang_key('proceed_to_login_page'); ?></a><?php } ?>
						</td>
					</tr>															
			<?php } ?>
			</tbody>
