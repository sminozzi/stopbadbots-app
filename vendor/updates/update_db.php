...
// $LogBase->log('Another line');
    ...
    $table_name = DB_PREFIX  . "sbb_blacklist";
    ob_start();
    ...
    $fileUrl = "https://stopbadbots.com/api/httpapi.php";
    ...
    $q = count($r);
        for ($i = 0; $i < $q; $i++) {
          ...
          //  die();
            ...
            $botnickname = trim(strip_tags($r[$i]['botnickname']));
            $botname = trim(strip_tags($r[$i]['botname']));
            $botip = trim(strip_tags($r[$i]['botip']));
            $botua = trim(strip_tags($r[$i]['botua']));
