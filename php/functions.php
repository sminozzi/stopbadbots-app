...
*/
    ...
    $LogBase->log($x);
    */
    // $delta = 3600 * 24 * $ndays * 2;
    //$LogBase->log($delta);
    /*
    if ($delta > (3600 * 24 * $ndays)) {
        // $LogBase->log('Another line');
        if (file_exists(SBB_PATH . "/updates/update_db.php")) {
            // $LogBase->log('Another line');     
            require_once(SBB_PATH . "/updates/update_db.php");
        }
        if (file_exists(SBB_PATH . "/updates/update_db_ip.php")) {
            // $LogBase->log('Another line');     
            require_once(SBB_PATH . "/updates/update_db_ip.php");
        }
        //update_option('check_for_db_updates', time());
    }
    */
...
function cron_function_ip()
{
    ...
    // `botdate` timestamp DEFAULT CURRENT_TIMESTAMP,
...
function cron_function()
{
    ...
    $sql = "delete from " . $table_name . " WHERE `data` <  CURDATE() - interval 7 day";
    $result = $conn->query($sql);
}
...
function add_option($field, $field_content)
{
    ...
    $query = "INSERT INTO `$table_name` (`name`, `content`) VALUES ('$field', '$field_content')";
    $result = $conn->query($query);
    if (!$result)
        return false;
    return true;
...
/*
    if (!$result = $conn->query($query)) {
       ...
       var_dump("Query Error (" . $conn->errno . "): " . $conn->error);
       var_dump("Failed query: " . $query);
        
        // You can also display a user-friendly message or take alternative action
       // echo "An error occurred while fetching statistics.";
       //return false;
       die();
...
function sbb_fill_db_froma()
{
     ...
     */
            ...
            */
            ...
            $query = "INSERT INTO " . $table_name .
                " (botua, botblocked, botobs, botip, boturl, botnickname, botname, botstate, botflag, botdate)
  VALUES ('', 0, '', '', '', '" . $botnickname . "', '" . $botname . "', 'Enabled', '" . $botflag . "', NOW())";
...
function sbb_create_db()
{
    ...
    */
    $sql = "CREATE TABLE IF NOT EXISTS $table (
        `id` mediumint(9) NOT NULL AUTO_INCREMENT,
        `botnickname` varchar(30) DEFAULT NULL,
        `botname` text DEFAULT NULL,
        `boturl` text DEFAULT NULL,
        `botip` varchar(100) DEFAULT NULL,
        `botobs` text DEFAULT NULL,
        `botstate` varchar(10) DEFAULT NULL,
        `botblocked` mediumint(9) DEFAULT NULL,
        `botdate` timestamp DEFAULT CURRENT_TIMESTAMP,
        `botflag` varchar(1) DEFAULT NULL,
        `botua` text DEFAULT NULL,
        UNIQUE (`id`),
        UNIQUE (`botnickname`)
    )";
...
function sbb_create_db2()
{
    ...
    */
    ...
    `botblocked` mediumint(9) DEFAULT NULL,
    `botdate` timestamp DEFAULT CURRENT_TIMESTAMP,
    `added` varchar(30) DEFAULT NULL,
    `botflag` varchar(1) DEFAULT NULL,
    `botcountry` varchar(2) DEFAULT NULL,
    UNIQUE (`id`),
    UNIQUE (`botip`)
)";
...
function sbb_create_db4()
{
    ...
    */
    ...
    $sql = "CREATE TABLE $table (
        ...
        `cookie` varchar(1) DEFAULT NULL,
        `response` varchar(5) DEFAULT NULL,
        `bot` varchar(1) DEFAULT NULL,
        UNIQUE (`id`)
    )";
...
function sbb_create_db_stats()
{
    ...
    */
    ...
    $sql = "CREATE TABLE " . $table . " (
        ...
        `qnick` text DEFAULT NULL,
        `qip` text DEFAULT NULL,
        `qfire` text DEFAULT NULL,
        `qref` text DEFAULT NULL,
        `qping` text DEFAULT NULL,
        `quenu` text DEFAULT NULL,
        `qlogin` text DEFAULT NULL,
        `qcom` text DEFAULT NULL,  
        `qcon` text DEFAULT NULL,         
        `qua` text DEFAULT NULL,
        `qfalseg` text DEFAULT NULL,
        `qtools` text DEFAULT NULL,
        `qrate` text DEFAULT NULL,           
        `qother` text DEFAULT NULL,
        `qtotal` varchar(100) DEFAULT NULL,
        UNIQUE (`id`),
        UNIQUE (`date`)
    )";
